var searchInput = document.querySelector("#search-focus")
        ...
        .then(function(mediaData){
            if(!mediaData.data){
                document.querySelector("#searchMedia").innerHTML=""
                 document.querySelector("#searchMedia").style.background="none"
            }else{
                var clutter = ""
                mediaData.data.forEach(function(alldata){
                console.log(alldata)
                clutter+=`<h3>${alldata.description}</h3>`
                document.querySelector("#searchMedia").innerHTML=clutter
                document.querySelector("#searchMedia").style.background="white"
            })
            }                 
        })
    }else{
...
function CheckAndSubmit () {
    var searchField = document.getElementById ("search-focus");
...
document.querySelector("#title-upload-btnnn").addEventListener("change",function(){
document.querySelector("#exampleModal").style.display = "none"
